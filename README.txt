...
v0.59
- ACK受信時の320×320固定縮小を廃止
- 送信側Inカメラの中央正方形を取得解像度のままjsQRへ入力
- ACK QRの4隅配置・表示サイズ・ECC=Lはv0.58のまま
- Inカメラ要求解像度640×480も変更なし


v0.60
- 位置合わせ時の送信側赤枠について時間保持を廃止
- ACKをそのスキャンで認識した時だけ赤枠を表示
- ACKを認識しなかったスキャンでは直ちに赤枠を消去
- 受信側ACK表示の1秒保持は変更なし
- ACK QRの4隅配置・サイズ・ECC・カメラ設定は変更なし


v0.61
- 送信側ACK検出処理を受信側QR検出処理と同じ方式へ統一
- 送信側Inカメラの640×480希望指定を廃止し、受信側と同じfacingMode指定のみへ変更
